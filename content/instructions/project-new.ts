...
3. Before building, confirm each screen has a component-provenance map for shell, tables/data surfaces, metrics/status surfaces, forms, overlays, and detail panels. Anything without a canonical NOS component or approved composition stays in "Needs review" until a human decides.
4. Build Phase 1 screens first, verify in the browser, then proceed.
5. Import NOS components from the local copied component path, for example: \`import { Button } from "@/components/ui/button"\`.
6. Use Tailwind classes that reference NOS CSS variables. Never hardcode colors, spacing, radius, shadow, or motion values.`
